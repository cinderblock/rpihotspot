...
function setWlanDetails() {
	...
	read wlanIpAddr wlanIpMask wlanIpCast <<< $(ifconfig ${wlanInterfaceName} | grep 'inet ' | awk -F " " '{print $2" "$4" "$6}')
		...
		if [ ! -z "$apCountryCodeTemp" ]; then
			if [[ "${countryCodeArray[@]}" =~ "${apCountryCodeTemp}" ]]; then
				if [[ \
					! -z "${wlanCountryCode}" && \
					(( ! "${countryCodeArray[@]}" =~ "${wlanCountryCode}") || \
					( ! "${apCountryCodeTemp}" =~ "${wlanCountryCode}")) \
				]]; then
...
doRemoveApIpEntriesFromHostFile() {
	if [ \
		`cat /etc/hosts | grep -c ^10.` -gt 0 -o \
		`cat /etc/hosts | grep -c ^172.` -gt 0 -o \
		`cat /etc/hosts | grep -c ^192.168.` -gt 0 \
	]; then
...
doInstall() {
	...
	if [ $(curl -Is http://www.google.com 2>/dev/null | head -n 1 | grep -c '200 OK') -gt 0 ]; then
		...
		if [ \
			-f $downloadDir/1_libnl-route-3-200.deb -a \
			...
			-f $downloadDir/7_iptables-persistent.deb \
		]; then
...
if [ "$install" = true -o "$installUpgrade" = true ]; then
	if [ \
		"$apSsidValid" = false -o \
		"$apPassphraseValid" = false -o \
		"$apCountryCodeValid" = false -o \
		"$apIpAddrValid" = false \
	]; then
		echo <<- EOF
